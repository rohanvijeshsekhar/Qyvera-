...
function _lockMobileScroll() {
  document.addEventListener("touchmove", _preventTouchScroll, {
    passive: false,
  });
...
master
  ...
  .add(() => {
    ...
    document
      .getElementById("hero-tagline")
      ?.style.setProperty("will-change", "opacity, clip-path");
    document
  ...
  .add(() => {
    ...
    gsap.set(pContent, {
      transformOrigin: `${visualCenterX}px ${visualCenterY}px`,
    });
    ...
    const targetVisualCenterY =
      targetBottom - (pContent.offsetHeight * scale) / 2;
  ...
  .add(() => {
    document.querySelectorAll(".ch-top").forEach((el) => {
...
async function setupScrollReveal() {
  ...
  [pContent, pLogo, pLuke, pBaffait, pDot].forEach((el) =>
  ...
  gsap.set(
    phraseChars,
    isMobile ? { opacity: 0 } : { opacity: 0, filter: "blur(10px)" },
  );

  const FRAME_DIR = "assets/images/hero%20sequence/hero1/";
  const FRAME_EXT = ".jpg";
  const FRAME_PAD = 3;
  const TOTAL_FRAMES = 100;
  const FRAME_CACHE_KEY = "20260416-r2";
  const frameUrl = (n) =>
    `${FRAME_DIR}ezgif-frame-${String(n).padStart(FRAME_PAD, "0")}${FRAME_EXT}?v=${FRAME_CACHE_KEY}`;
  ...
  function drawFrame(i) {
    ...
    ctx.drawImage(img, (cw - dw) * 0.5 - (cw * 0.12), (ch - dh) * 0.5, dw, dh);
    drawnIdx = i;
  }
  function probe(n) {
  ...
  async function loadFirstBatch() {
    ...
    if (!first) {
      console.error("Frame 1 failed to load!");
      return 0;
    }
    frames[0] = first;
    ...
    const batchNums = Array.from(
      { length: Math.min(SPEED_BATCH, TOTAL_FRAMES - 1) },
      (_, k) => k + 2,
    );
    await Promise.all(
  ...
  loadRemainingFrames(frameSkip).catch((err) =>
    console.error("Frame background load error:", err),
  );
  ...
  scrollTl.fromTo(
    ...
    { x: introXvw, y: 0, duration: 0.3, ease: "none" },
    0,
  );
  scrollTl.fromTo(
    "#hero-tagline",
  ...
  scrollTl.fromTo(
    pLogo,
    { x: "0vw", opacity: 1 },
    { x: exitLeft, opacity: 0, duration: 0.7, ease: "none" },
    0.3,
  );
  scrollTl.fromTo(
    pLuke,
  ...
  scrollTl.fromTo(
    pBaffait,
    { x: "0vw", opacity: 1 },
    { x: exitRight, opacity: 0, duration: 0.7, ease: "none" },
    0.3,
  );
  scrollTl.fromTo(
    pDot,
  ...
  ScrollTrigger.create({
    ...
    onUpdate: (self) => {
      ...
      const phase2 = Math.min(
        1,
        Math.max(0, (p - REVEAL_PHASE_START) / REVEAL_PHASE_DURATION),
      );
  ...
  if (
    ...
    exitTl.to(
      revealOverlay,
      { backdropFilter: "blur(16px)", ease: "none", duration: 1 },
      0,
    );
  }
  ...
  ScrollTrigger.create({
    ...
    onUpdate: (self) => {
      const exitFrameProgress =
        FRAME_PROGRESS_AT_EXIT_START +
        self.progress * (1 - FRAME_PROGRESS_AT_EXIT_START);
...
function setupAboutSection() {
  ...
  if (isMobile) {
    [
      ...
      ...aboutVersion.querySelectorAll(".word"),
    ].forEach((w) => {
      w.style.filter = "none";
    });
  }
  ...
  gsap.set(
    aboutSub,
    isMobile ? { opacity: 0 } : { opacity: 0, filter: "blur(12px)" },
  );
  gsap.to(aboutSub, {
    opacity: 1,
    ...(isMobile ? {} : { filter: "blur(0px)" }),
...
function setupProjectsSection() {
  ...
  items.forEach((item, i) => {
    item.addEventListener("click", () => {
      if (item.classList.contains("active")) {
        ...
        lenis.scrollTo(
          docTop - window.innerHeight / 2 + item.offsetHeight / 2,
          { duration: 1.2 },
        );
      }
    });
  ...
  cover.addEventListener("click", () => {
    if (currentIdx >= 0)
      openProject(items[currentIdx].dataset.id, items[currentIdx]);
  });
  ...
  const qCursorX = gsap.quickTo(projCursor, "left", {
    duration: 0.35,
    ease: "power3.out",
  });
  const qCursorY = gsap.quickTo(projCursor, "top", {
    duration: 0.35,
    ease: "power3.out",
  });
  let _tiltTargetRY = 0,
  ...
  document.addEventListener("mousemove", (e) => {
    ...
    if (projectOpen && _cachedSelImg) {
      ...
      const drx = Math.max(
        -1,
        Math.min(1, (e.clientY - scy) / (sr.height / 2)),
      );
  ...
  gsap.ticker.add(() => {
    ...
    if (projectOpen && _cachedSelImg) {
      ...
      _cachedSelImg.style.transform =
        "rotateY(" +
        _detTiltRY.toFixed(2) +
        "deg) rotateX(" +
        _detTiltRX.toFixed(2) +
        "deg)";
    }
  });
    ...
    (function buildSlices() {
      ...
      document.querySelectorAll(".cg-img").forEach(function (img) {
        ...
        for (var s = 0; s < SLICES; s++) {
          ...
          sl.style.backgroundSize =
            imgW.toFixed(1) + "px " + imgH.toFixed(1) + "px";
  ...
  (function () {
    ...
    var zoneTop =
      document.getElementById(sections[0].id).getBoundingClientRect().top +
      scrollY0;
    var lastEl = document.getElementById(sections[sections.length - 1].id);
    var zoneBottom =
      lastEl.getBoundingClientRect().top + lastEl.offsetHeight + scrollY0;
      ...
      onUpdate: function (self) {
        ...
        for (var i = 0; i < sections.length; i++) {
          ...
          if (progress < segEnd) {
            ...
            segEls[i].fill.style.height =
              (Math.min(1, Math.max(0, inner)) * 100).toFixed(1) + "%";
            activeIdx = i;
  ...
  (function () {
    ...
    var frameYEnd = function () {
      return -window.innerHeight * 1.4;
    };
    var dispoY = function () {
      return window.innerHeight * 1.1;
    };
    var dispoYEnd = function () {
    ...
    tl.to(
      frame2,
      { y: frameYEnd, duration: frameDur, ease: "none" },
      pairStart + 0.07,
    );
    tl.to(
      frameImg2,
  ...
  (function () {
    ...
    function setupHover(preEl, poolGrid) {
      ...
      function init() {
        ...
        origGrid = origLines.map(function (l) {
          return l.split("");
        });
      }
      ...
      preEl.addEventListener("mousemove", function (e) {
        ...
        for (var y = yMin; y <= yMax; y++) {
          for (var x = xMin; x <= xMax; x++) {
            ...
            if (
              dx * dx + dy * dy <
              (radius + noise[y][x]) * (radius + noise[y][x])
            ) {
              hitTime[y][x] = now;
            }
          }
        ...
        for (var y = 0; y < rows; y++) {
          for (var x = 0; x < cols; x++) {
            ...
            if (hitTime[y][x] > 0 && elapsed < cellDuration[y][x]) {
              ...
              html +=
                '<span style="color:#0a0a0a;background:#ff3b14">' +
                esc(ch) +
                "</span>";
            } else {
    ...
    function parallaxLoop() {
      ...
      var py = sy * -10;
      if (asciiLeftPre)
        asciiLeftPre.style.transform = "translate(" + lx + "px, " + py + "px)";
      if (asciiRightPre)
        asciiRightPre.style.transform = "translate(" + rx + "px, " + py + "px)";
    ...
    var footerTopChars = document.querySelectorAll(
      "#footer .footer-top .chr-hover .ch-top",
    );
    ...
    ftl.to(
      ...
      {
        y: function () {
          return -(window.innerHeight * 1.8 + 400);
        },
        immediateRender: false,
        duration: 1.0,
        ease: "none",
  ...
  const PROJECTS = {
    cyberdiag: {
      desc: isEn
        ? "An intelligent healthcare platform focused on digital interaction and smart recommendations."
        : "Une plateforme de santé intelligente axée sur les interactions numériques et les recommandations personnalisées.",
    ...
    anima: {
      desc: isEn
        ? "A modern analytics system transforming business data into actionable insights."
        : "Un système analytique moderne transformant les données d'entreprise en informations exploitables.",
    ...
    "cyberdiag-app": {
      desc: isEn
        ? "A streamlined customer management platform built for growing businesses."
        : "Une plateforme de gestion client simplifiée conçue pour les entreprises en croissance.",
    ...
    zenith: {
      desc: isEn
        ? "A premium ecommerce experience developed for modern dental product distribution."
        : "Une expérience e-commerce haut de gamme développée pour la distribution de produits dentaires.",
    ...
    skymcdb: {
      desc: isEn
        ? "A scalable platform for managing products, logistics, and operations."
        : "Une plateforme évolutive pour gérer les produits, la logistique et les opérations.",
    ...
    chromablock: {
      desc: isEn
        ? "A secure modern portal designed for workflow and client communication."
        : "Un portail moderne et sécurisé conçu pour la communication client et le suivi de flux.",
    ...
    symphony: {
      desc: isEn
        ? "A reliable and high-performance cloud provisioning and management platform."
        : "Une plateforme fiable et performante de provisionnement et gestion d'infrastructures cloud.",
    ...
    echo: {
      desc: isEn
        ? "An all-in-one resource planning platform optimized for business automation."
        : "Une plateforme tout-en-un de planification de ressources optimisée pour l'automatisation d'affaires.",
      category: isEn ? "Enterprise" : "Entreprise",
      year: "2026",
  ...
  function openProject(id, clickedItem) {
    ...
    if (stTimeline)
      stTimeline.style.setProperty("display", "none", "important");
    ...
    detailTags.innerHTML = proj.tags
      .map(function (t) {
    ...
    detailThumbsInner.innerHTML = allImages
      .map(function (src) {
        return '<img src="' + src + '" alt="" decoding="async">';
      })
      .join("");
    detailSelected.innerHTML =
      '<img src="' + allImages[0] + '" alt="" decoding="async">';
    ...
    tl.add(function () {
      _galleryMaxScroll = Math.max(
        0,
        detailThumbsInner.scrollHeight - detailThumbs.clientHeight,
      );
  ...
  function closeProject() {
    ...
    window._projectOpen = false;
    if (_galleryRAF) {
      cancelAnimationFrame(_galleryRAF);
      _galleryRAF = null;
    }
    _qGalleryY = null;
    ...
    tl.to(
      [detailDesc, detailTags, detailBack],
  ...
  function preloadProjectImages() {
    ...
    items.forEach(function (item) {
      if (item.dataset.img) allSrcs.push(item.dataset.img);
    });
    ...
    var seen = {};
    allSrcs = allSrcs.filter(function (s) {
